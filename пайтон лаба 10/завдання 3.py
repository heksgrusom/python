...
def showGrathPie(dictionary):
    ...
    for key,value in dictionary.items():
        if value > 0:
            ...
            snowing+=1
    y = np.array([raining,snowing])
    mylabels = ["Raining days", "Snowing days"]
    mycolors = ["blue", "gray"]
...
writeToJson()
while(True):
    x = int(input("Enter number - (1 for show json info) (2 add info) (3 delete info) (4 search info about weather of day) (5 show result info) (6 show pie grath) 7 exit\n"))
    if (x == 1):
        readFromJson()
        print(dict)
    elif (x == 2):
        ...
        for key,value in dict.items():
            if value > 0:
                dict_r[key] = "Outside temperature was - {}.It was raining outside".format(value)
            else:
                dict_r[key] = "Outside temperature was - {}.It was snowing outside".format(value)
        try:
            with open("result.json", "w") as write_file:
                json.dump(dict_r, write_file)
        except FileExistsError:
            print("file not exist")
        try:
